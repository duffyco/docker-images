...
# setup defaults for /etc/amportal.conf
echo "AMPDBUSER=asteriskuser" >> /etc/amportal.conf
echo "AMPDBPASS=amp109" >> /etc/amportal.conf
echo "AMPDBHOST=localhost" >> /etc/amportal.conf
echo "AMPDBNAME=asterisk" >> /etc/amportal.conf
echo "AMPDBENGINE=mysql" >> /etc/amportal.conf
echo "ASTMANAGERHOST=localhost" >> /etc/amportal.conf
echo "AMPMGRUSER=admin" >> /etc/amportal.conf
echo "AMPMGRPASS=amp111" >> /etc/amportal.conf
echo "AMPWEBROOT=/var/www/html" >> /etc/amportal.conf
...
#wget http://downloads.asterisk.org/pub/telephony/asterisk/releases/asterisk-13.10.0.tar.gz
...
wget http://incrediblepbx.com/flite-ubuntu14.tar.gz
...
expect -c 'set timeout 60;spawn apt-get install anacron -y;expect "default=N";send "\r\r";interact'
if [ $ubuntu != 686 ] ; then
# 64-bit stuff goes here
...
rm -f upgrade-asterisk13-to-current-rhel
...
# Click-to-Dial Patch for AsteriDex from @jerrm
sed -i 's|&$errno|$errno|' /var/www/html/asteridex4/callboth.php
sed -i 's|&$errstr|$errstr|' /var/www/html/asteridex4/callboth.php

# Patch to replace GoogleTTS with FLITE
