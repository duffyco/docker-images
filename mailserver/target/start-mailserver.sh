...
function _setup_dovecot() {
	...
	sed -i -e 's/#ssl = yes/ssl = required/g' /etc/dovecot/conf.d/10-ssl.conf
        ...
        cp -a /tmp/docker-mailserver/ssl-parameters.dat /var/lib/dovecot 
...
function _setup_dovecot_local_user() {
	...
	if [ -f /tmp/docker-mailserver/postfix-accounts.cf -a "$ENABLE_LDAP" != 1 ]; then
		...
		do
			...
			echo "${login}:${pass}:507:507::/var/mail/${domain}/${user}::" >> /etc/dovecot/userdb
...
function _setup_postfix_aliases() {
	...
	echo -n > /etc/postfix/regexp
        ...
        cp -f /tmp/docker-mailserver/relay_passwd /etc/postfix/relay_passwd
	...
	if [ -f /tmp/docker-mailserver/postfix-regexp.cf ]; then
        ...
        #Procmail Setup
        cp -f /tmp/docker-mailserver/default.rc /etc/postfix/default.rc
        chown root:mailusers /etc/postfix/default.rc
        mkdir -p /var/log/mail
        touch /var/log/mail/procmail.log
        chown root:mailusers /var/log/mail/procmail.log
        chmod g+w /var/log/mail/procmail.log
...
"self-signed" )
	...
	if [ -e "/tmp/docker-mailserver/ssl/$HOSTNAME/$HOSTNAME-cert.pem" ] \
	...
	&& [ -e "/tmp/docker-mailserver/ssl/cacert.pem" ]; then
		...
		cp "/tmp/docker-mailserver/ssl/$HOSTNAME/smtp.pem" /etc/postfix/ssl
		...
		# Postfix configuration
                ...
                sed -i -r 's~#smtp_tls_CAfile=~smtp_tls_CAfile=/etc/postfix/ssl/smtp\.pem~g' /etc/postfix/main.cf
                ln -s /etc/postfix/ssl/cacert.pem "/etc/ssl/certs/cacert-$HOSTNAME.pem"

                # Dovecot configuration
                sed -i -e 's~\#ssl_ca =~ssl_ca = </etc/postfix/ssl/cacert\.pem~g' /etc/dovecot/conf.d/10-ssl.conf
...
function _setup_security_stack() {
  ...
  echo "# WARNING: this file is auto-generated." > $dms_amavis_file
	...
	sed -i -e 's~invoke-rc.d spamassassin reload~/etc/init\.d/spamassassin reload~g' /etc/cron.daily/spamassassin
        ...
        #JD: Fix spamassin
        chown root:amavis /etc/spamassassin/spamassassin-rules.cf 
...
#	if [ `find /var/mail -maxdepth 3 -a \( \! -user 507 -o \! -group 507 \) | grep -c .` != 0 ]; then
#		notify 'inf' "Fixing /var/mail permissions"
#		chown -R 507:507 /var/mail
#	else
